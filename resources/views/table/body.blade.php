    ...
    @forelse($table->resource as $itemKey => $item)
        ...
        >
            @if($hasBulkActions = $table->hasBulkActions())
                ...
                <td width="64" class="text-xs px-6 py-4" :class="{'bg-zinc-200 dark:bg-zinc-600': table.itemIsSelected(@js($itemPrimaryKey))}">
                    <div class="h-full">
                        <input
                            ...
                            class="dark:bg-zinc-500 border-zinc-300 rounded text-primary-600 shadow-sm focus:border-primary-300 focus:ring focus:ring-primary-200 focus:ring-opacity-50 disabled:opacity-50"
            ...
            @foreach($table->columns() as $column)
                @if($column->key === 'actions')
                    <td
                        ...
                        "
                        class="table_action md:sticky w-0 last:before:hidden md:last:before:block md:last:before:h-full md:last:before:top-0 md:ltr:last:before:left-[-15px] md:rtl:last:before:right-[-15px] md:last:before:absolute md:last:before:w-[15px] ltr:last:border-l last:border-zinc-200 rtl:last:border-r dark:last:border-zinc-700 rtl:left-0 ltr:right-0 bg-white dark:bg-zinc-800 table_action_hover whitespace-nowrap ltr:capitalize text-sm @if($loop->first && $hasBulkActions) pr-6 @else px-6 @endif py-4 @if($column->highlight) text-zinc-900 dark:text-white font-medium text-left rtl:text-right @else text-zinc-500 dark:text-zinc-200 @endif @if($table->rowLinks->has($itemKey)) cursor-pointer @endif {{ $column->classes }}"
                    >
                ...
                @else
                    <td
                        ...
                        class=" table_action_hover whitespace-nowrap ltr:capitalize text-sm   @if($loop->first && $hasBulkActions) pr-6 @else px-6 @endif py-4 @if($column->highlight) text-zinc-900 dark:text-white font-medium text-left rtl:text-right @else text-zinc-500 dark:text-zinc-200 @endif @if($table->rowLinks->has($itemKey)) cursor-pointer @endif {{ $column->classes }}"
                    >
                        @isset(${'spladeTableCell' . $column->keyHash()})
                            {{ ${'spladeTableCell' . $column->keyHash()}($item, $itemKey) }}
                        @else
