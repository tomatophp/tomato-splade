<thead class="bg-zinc-50 dark:bg-zinc-700">
    <tr>
        @if($hasBulkActions = $table->hasBulkActions())
            <th width="64" class="text-xs px-6 py-4">
        ...
        @foreach($table->columns() as $column)
            <th
                ...
                class="md:last:sticky last:before:hidden md:last:before:block md:last:before:h-full md:last:before:top-0 md:ltr:last:before:left-[-15px] md:rtl:last:before:right-[-15px] md:last:before:absolute md:last:before:w-[15px] ltr:last:border-l last:border-zinc-200 rtl:last:border-r dark:last:border-zinc-700 ltr:last:right-0 rtl:last:left-0 last:bg-zinc-50 last:dark:bg-zinc-700 @if($loop->first && $hasBulkActions) pr-6 @else px-6 @endif py-3 text-left rtl:text-right text-xs font-medium tracking-wide text-zinc-500 dark:text-white {{ $column->classes }}"
            >
                @if($column->sortable)
